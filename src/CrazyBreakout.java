...
public class CrazyBreakout extends GraphicsProgram {
	...
	private void buildWallOfBricks() {
		...
		for (int i = 0 ; i < NBRICK_ROWS ; i++){
			...
			for (int j = 0 ; j < NBRICKS_PER_ROW ; j++){
				wallOfBricks [i][j] = new GRectAnimated (x, y, BRICK_WIDTH, BRICK_HEIGHT );
				...
				Thread t = new Thread (wallOfBricks [i][j]);
				t.start();
...
/** */	
	private void updateBall () {
		...
		pauseTime = 1000/ frames_per_second;
		...
		if ((getElementAt(ballUpLeftCorner) != null)){
			...
			if (lastObj != null){
				...
				if (lastObj != paddle) {
					remove(lastObj);
					remainingBricks--;
					frames_per_second += increseSpeed;
				}
			}
...
/** */
	private void updatePainel(){
		String str = remainingBricks + "bricks Remaining e : " + turns + " tentativas " ;
		painel.setLabel(str);
...
/**	*/
	private void result(){
		...
		if (remainingBricks > 0 && turns == 0){
			
			GLabel finish = new GLabel ("Finish");
			finish.setFont("Arial-24");
			
			int x = (int)(APPLICATION_WIDTH  - finish.getWidth ()) / 2;
			int y = (int)(APPLICATION_HEIGHT - finish.getHeight()) / 2;
			
			add(finish, x, y);
			
			
		} else if (remainingBricks == 0 && turns > 0){
		
			GLabel win = new GLabel ("You Win");
			win.setFont("Arial-24");
			
			int x = (int)(APPLICATION_WIDTH  - win.getWidth ()) / 2;
			int y = (int)(APPLICATION_HEIGHT - win.getHeight()) / 2;
...
/**	An Array thar holds the wall of Bricks*/
	private GRectAnimated [][] wallOfBricks = new GRectAnimated [NBRICK_ROWS][NBRICKS_PER_ROW];
...
/**	*/
	private int pauseTime = 1000/ frames_per_second;

/** */
	private int increseSpeed = 10;
